...
def multifold(iterations,
    ...
    sim_reco_mask = np.ones(len(sim_reco)).astype(bool)
    for i in range(num_observables):
        sim_reco_mask *= sim_reco[:, i]!=-99
        
    sim_truth_mask = np.ones(len(sim_truth)).astype(bool)
    for i in range(num_observables):
        sim_truth_mask *= sim_truth[:, i]!=-99
    
    data_reco_mask = np.ones(len(data_reco)).astype(bool)
    for i in range(num_observables):
        data_reco_mask *= data_reco[:, i]!=-99
    
    xvals_1 = np.concatenate((sim_reco[sim_reco_mask], data_reco[data_reco_mask]))
    yvals_1 = np.concatenate(
        (labels_sim[sim_reco_mask], labels_data[data_reco_mask]))

    if dummyval is not None:
        if average_weight:
            sim_mask = sim_reco_mask * sim_truth_mask
            xvals_1b = np.concatenate((sim_truth[sim_mask],
                                       sim_truth[sim_mask]))
    ...
    xvals_2 = np.concatenate((sim_truth[sim_truth_mask],
                              sim_truth[sim_truth_mask]))
