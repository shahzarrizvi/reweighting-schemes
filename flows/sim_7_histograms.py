...
dt = np.vstack([dt_pt, dt_eta, dt_m, dt_w, dt_sdms, dt_tau21]).T
...
experiment = 'flows/sim/7'

ckpt = torch.load('flows/sim/7/ckpt_best')
...
ratio_hist(mc[:, 4], smp[:, 4], 
           labels = ['MC', 'Flow', r'$m_{\rm SD}$'],
           color = 'blue',
           figsize = (w, h),
           title = r'$m_{\rm SD}$ (MC)',
           x_lim = (0, 80),
           filename = '../plots/zenodo/flows/{}/sdm.png'.format(experiment)
          )
...
ratio_hist(mc[:, 5], smp[:, 5], 
           ...
           title = r'$\tau_{21}$ (MC)',
           x_lim = (0, 1.2),
           filename = '../plots/zenodo/flows/{}/tau21.png'.format(experiment)
          )
