...
def nmlc(a, b):
    ...
    if a == 0:
        return integrate.quad(g, -np.inf, np.inf)[0]
    return integrate.quad(g, -b/a, np.inf)[0]
...
#nbce_zs = np.zeros(g.shape[0])
#nmse_zs = np.zeros(g.shape[0])
nmlc_zs = np.zeros(g.shape[0])
nsqr_zs = np.zeros(g.shape[0])

for i in range(g.shape[0]):
    #nbce_zs[i] = nbce(g[i][0], g[i][1])
    #nmse_zs[i] = nmse(g[i][0], g[i][1])
    nmlc_zs[i] = nmlc(g[i][0], g[i][1])
    nsqr_zs[i] = nsqr(g[i][0], g[i][1])
    if i % 100 == 0:
        #print(i, ':', nbce_zs[-1], nmse_zs[-1], nmlc_zs[-1], nsqr_zs[-1]) 
        print(i, g[i], '\t:', nmlc_zs[i], '\t', nsqr_zs[i]) 
    i += 1
