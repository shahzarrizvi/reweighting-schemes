...
The $\pt$, $y$, $\phi$, and the number of constituents distributions for the leading and subleading track jet are shown in figure~\ref{fig:pTyjets}, and figure~\ref{fig:jetsubstructure} shows some
selected jet substructure variables. The substructure variables are the $n$-subjettiness observables~\cite{Thaler:2010tr,Thaler:2011gf}, which have the form
...
The MC simulation for both \powheg+\pythia~and \sherpa~2.2.1 is expected to underpredict higher order processes, leading to a discrepancy between the MC and data due to the high $\pt^{\mu\mu}$ phase space.\footnote{This underprediction by the Monte Carlo has been observed in other $Z+$jets analyses, see for example~\cite{ATLAS:2019zci} or~\cite{Blumenschein:2744872}, which examines a similar phase space (note that \sherpa~2.2.1 and \sherpa~2.2.11 exhibit nearly identical distributions for the observables presented in this analysis).}.
...
\begin{figure}[h!]
  ...
  \caption{The ``actual'' mean interactions per bunch crossing, $\mu$, for 2015--2016 data (top left), 2017 data (top right), 2018 data (bottom left), and the full Run-2 dataset (bottom right). Note that the pileup reweighting uses actual $\mu$ for 2017 and 2018 data, while $\mu_\mathrm{avg}$, i.e.\ $\mu$ calculated by averaging the actual $\mu$ of all bunches, is used for 2015--2016 data hence a worse agreement is seen for this period. Note that the error bars reflect only the statistical uncertainty on the data or MC.}
  \label{fig:MuActual}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{Distributions for the $\pt$, $y$, and $m$ of the dilepton system. Due to the high $\pt$ phase space, it is expected that the MC simulation will underpredict the data. Note that the error bars reflect only the statistical uncertainty on the data or MC.}
  \label{fig:DilepVars}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[page=24,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=28,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf} \\
  \includegraphics[page=32,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=36,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf} \\
  \includegraphics[page=40,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=44,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}
  \caption{The $\pt$, $\eta$, and $\phi$ distributions for the leading muon (left) and the subleading muon (right). Note that the error bars reflect only the statistical uncertainty on the data or MC.}
  \label{fig:pTetamus}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[page=72,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=88,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf} \\
  \includegraphics[page=76,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=92,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf} \\
  \includegraphics[page=80,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=96,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf} \\
  \includegraphics[page=136,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}\includegraphics[page=140,width=0.45\textwidth]{figures/ZjetOmnifoldMCDataComp.pdf}
  \caption{The $\pt$, $y$, $\phi$, and the number of constituents distributions for the leading (left) and the subleading (right) track jet. Note that the error bars reflect only the statistical uncertainty on the data or MC.}
  \label{fig:pTyjets}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{Jet substructure features for the leading (left) and sub-leading (right) track jets. Note that the error bars reflect only the statistical uncertainty on the data or MC.}
...
\begin{figure}[h!]
  ...
  \caption{Distributions for the number of tracks per event (top left), and the $\pt$ (top right), $\eta$ (bottom left), and $\phi$ (bottom right) for each reconstructed track. Note that the error bars reflect only the statistical uncertainty on the data or MC.}
  \label{fig:trackInfo}
\end{figure}
