...
The event selection is outlined in table~\ref{lab:eventsel}.
It is primarily designed to reject background events in favour of the $(Z\to\mu\mu)$+jets signal process.
Following the selection, contributions from processes other than strong $Z$+jet production are negligible as can be seen in appendix~\ref{app:bkg-processes}.
The various criteria are discussed in more detail below.
...
\begin{table}[h!]
    ...
    Muons & $N_{\text{muons}}\geq2$ \\
          ...
          & $81\leq m_{\mu\mu} (\text{GeV})\leq 101$ \\
          & $\pt^{\mu\mu}\geq200$ GeV \\ \hline
    \end{tabular}
...
where BL stands for beamline, and $d_0$ and $z_0$ represent the point along the track with the closest approach to the beamspot.

The large $\pt^{\mu\mu}$ cut of 200~GeV has been chosen to focus on events in which the balancing jet(s) have a high quantity of charged particles and to reduce the overall dataset size. Events with a $\pt^{\mu\mu}$ larger than 165~GeV are saved in order to study migration effects between truth and reconstructed events (i.e. events where the reconstructed $\pt^{\mu\mu}$
...
\paragraph{Tracks} are reconstructed from charged-particle hits within the silicon- and straw-tube based inner tracking detectors. They are required to have a $\pt>500$~\MeV~and to pass the Loose quality working point. They must also be associated with the primary vertex of the event using the Tight track-to-vertex association\footnote{Information on both the selection tool and the TTVA tool may be found here: \url{twiki: https://twiki.cern.ch/twiki/bin/view/AtlasProtected/TrackingCPRecsRun2Final}} (see appendix~\ref{app:jetcomp}).
The primary vertex is defined as the reconstructed vertex with the highest sum of associated track $\pt^2$. The track $\eta$ and $\phi$ coordinates come from the five-parameter track fit to $(\eta,\phi,q/p,d_0,z_0)$, and thus correspond to the track coordinates at the origin.
...
Track jets are utilized in the analysis as uncertainties are available at a per-constituent level (see section~\ref{subsec:Tracksysts} for a full description of the track uncertainties). For jets, there exist no well-defined uncertainties for the neutral clusters and thus per-constituent uncertainties are impossible to define. Having access to per-constituent uncertainties allows for the derivation of errors for any general jet observable, including jet substructure quantities, that would not be possible or robust otherwise.
